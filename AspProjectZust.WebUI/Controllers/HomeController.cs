...
namespace AspProjectZust.WebUI.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> TodayBirhdayFriends()
        {
            ...
            var todayBirthdayFriend = friends.Where(f => f.DateOfBirth.Month == DateTime.Now.Month && f.DateOfBirth.Day == DateTime.Now.Day).ToList();
            return Ok(todayBirthdayFriend);
        }
        ...
        public async Task<IActionResult> SettingInAccount(UpdateUserViewModel updateUser)
        {
            var user = await _userManager.GetUserAsync(HttpContext.User);
            var country = await _dbContext.UserCountries.FirstOrDefaultAsync(f => f.Id == updateUser.Country);
        ...
        public async Task<IActionResult> SettingChangePassword(UpdateUserViewModel model)
        {
            var user = await _userManager.GetUserAsync(HttpContext.User);
            if (ModelState.IsValid && model.NewPassword == model.ChangePassword)
            {
                var signIn = await _signInManager.PasswordSignInAsync(user.UserName, model.CurrentPassword, false, false);
                if (signIn.Succeeded)
                {
                    var newPassword = HashPassword(model.NewPassword);
        ...
        public async Task<IActionResult> GetSearchFriends(string filter)
        {
            var user = await _userManager.GetUserAsync(HttpContext.User);
            var myFriends = await _dbContext.Friends.Where(f => f.YourFriendId == user.Id).ToListAsync();
            var data = await _dbContext.Users.Where(i => i.Id != user.Id).ToListAsync();
            ...
            var searchFriends = friends.Where(f => f.UserName.ToLower().Contains(filter.ToLower().Trim())).ToList();
            return Ok(searchFriends);
        }
        ...
        public async Task<IActionResult> Setting()
        {
            ...
            var relationStatus = await _dbContext.UserRelationStatus.FirstOrDefaultAsync(f => f.Status == user.RelationStatus);
            var occupation = await _dbContext.UserOccupations.FirstOrDefaultAsync(f => f.Name == user.Occupation);
            var language = await _dbContext.UserLanguages.FirstOrDefaultAsync(f => f.Tongue == user.Language);
            var gender = await _dbContext.UserGenders.FirstOrDefaultAsync(f => f.Name == user.Gender);
            var bloodGroup = await _dbContext.UserBloodGroups.FirstOrDefaultAsync(f => f.Blood == user.BloodGroup);
            var city = await _dbContext.UserCities.FirstOrDefaultAsync(f => f.Name == user.City);
            var country = await _dbContext.UserCountries.FirstOrDefaultAsync(f => f.Name == user.Country);
            ...
            if (bloodGroup != null)
            {
                model.BloodGroup = bloodGroup.Id;
            }
            if (gender != null)
            {
                model.Gender = gender.Id;
            }
            if (language != null)
            {
            ...
            if (occupation != null)
            {
                model.Occupation = occupation.Id;
            }
            if (relationStatus != null)
            {
                model.RelationStatus = relationStatus.Id;
            }
            return View(model);
        }
        ...
        public async Task<IActionResult> SettingInUserInfo(UpdateUserViewModel updateUser)
        {
            var occupations = await _dbContext.UserOccupations.ToListAsync();
            var gender = await _dbContext.UserGenders.ToListAsync();
            var relationStatus = await _dbContext.UserRelationStatus.ToListAsync();
            var bloodGroup = await _dbContext.UserBloodGroups.ToListAsync();
            var language = await _dbContext.UserLanguages.ToListAsync();
            var city = await _dbContext.UserCities.ToListAsync();
            var country = await _dbContext.UserCountries.ToListAsync();

            var user = await _userManager.GetUserAsync(HttpContext.User);
            user.PhoneNumber = updateUser.PhoneNumber;
            user.Address = updateUser.Address;
            if (updateUser.Country != 0)
            {
                user.Country = country[updateUser.Country].Name;
            }
            user.Email = updateUser.Email;
            user.FirstName = updateUser.FirstName;
            user.LastName = updateUser.LastName;
            if (updateUser.City != 0)
            {
                user.City = city[updateUser.City - 1].Name;
            }
            if (updateUser.Gender != 0)
            {
                user.Gender = gender[updateUser.Gender - 1].Name;
            }
            if (updateUser.Occupation != 0)
            {
                user.Occupation = occupations[updateUser.Occupation - 1].Name;
            }
            if (updateUser.RelationStatus != 0)
            {
                user.RelationStatus = relationStatus[updateUser.RelationStatus - 1].Status;
            }
            if (updateUser.BloodGroup != 0)
            {
                user.BloodGroup = bloodGroup[updateUser.BloodGroup - 1].Blood;
            }
            if (updateUser.Language != 0)
            {
                user.Language = language[updateUser.Language - 1].Tongue;
            }
